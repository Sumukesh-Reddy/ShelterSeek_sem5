...
const Booking = require('./model/booking')(paymentConnection);
...
app.get('/api/users', async (req, res) => {
  try {
    ...
    if (accountType === 'traveller') {
      users = await Traveler.find({ email: email.toLowerCase() });
    } else if (accountType === 'host') {
      users = await Host.find({ email: email.toLowerCase() });
    } else {
      return res.status(400).json({ 
        status: 'fail', 
        message: 'Invalid accountType. Must be "traveller" or "host"' 
      });
    }
    ...
    res.json({ 
      status: 'success', 
      data: { 
        travelers: accountType === 'traveller' ? users : [],
        hosts: accountType === 'host' ? users : []
      } 
    });
  } catch (error) {
...
app.get('/api/user/raw', async (req, res) => {
  try {
      ...
      // Return the raw user data (excluding password for security)
      ...
      console.error('Error fetching raw user data:', error.message);
      res.status(500).json({ 
          status: 'error', 
...
app.patch('/api/update-profile-photo', async (req, res) => {
  try {
    ...
    // Check if the new image exists in the main bucket
    const newImageExists = await mainGfsBucket.find({ _id: new ObjectId(profilePhoto) }).toArray();
    if (newImageExists.length === 0) {
      return res.status(404).json({ status: "fail", message: "New profile photo not found in storage" });
    }
    
    // Delete old profile photo from main bucket if it exists
    if (user.profilePhoto && ObjectId.isValid(user.profilePhoto)) {
      try {
        await mainGfsBucket.delete(new ObjectId(user.profilePhoto));
    ...
    res.status(200).json({ 
      ...
      data: { 
        user: { 
          ...
          profilePhoto: `/api/images/${profilePhoto}` 
        } 
      } 
    });
  } catch (error) {
...
mongoose.connection.once('open', () => {
  mainGfsBucket = new GridFSBucket(mongoose.connection.db, { bucketName: 'images' });
  console.log('GridFSBucket initialized for main connection');
});

// Update the /api/images/:id endpoint
app.get('/api/images/:id', async (req, res) => {
  try {
    const imageId = new ObjectId(req.params.id);
    
    // Try main connection first (for profile photos)
    let image = await mainGfsBucket.find({ _id: imageId }).toArray();
    let bucket = mainGfsBucket;
    
    // If not found in main connection, try hostAdmin connection (for room photos)
    if (!image || image.length === 0) {
      image = await gfsBucket.find({ _id: imageId }).toArray();
      bucket = gfsBucket;
    }
    ...
    if (!image || image.length === 0) {
      ...
      // Return a default placeholder image instead of error
      const placeholderPath = path.join(__dirname, 'public/images/sai.png');
    ...
    readStream.on('error', (error) => {
      ...
      // Fallback to placeholder on stream error
      const placeholderPath = path.join(__dirname, 'public/images/sai.png');
      if (fs.existsSync(placeholderPath)) {
...
app.get('/api/debug/image/:id', async (req, res) => {
  try {
    ...
    // Check main connection (profile photos)
    const mainImage = await mainGfsBucket.find({ _id: imageId }).toArray();
    
    // Check hostAdmin connection (room photos)
    const hostAdminImage = await gfsBucket.find({ _id: imageId }).toArray();
    
    res.json({
      imageId: req.params.id,
      existsInMain: mainImage.length > 0,
      existsInHostAdmin: hostAdminImage.length > 0,
      mainImage: mainImage[0] || null,
      hostAdminImage: hostAdminImage[0] || null
    });
  } catch (error) {
