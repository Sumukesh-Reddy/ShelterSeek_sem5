...
function populateHostRequests(filteredRequests = window.hostRequests) {
    ...
    filteredRequests.forEach((request, index) => {
        ...
        const isExpired = createdAtDate && (Date.now() - createdAtDate.getTime() >= ninetyDaysMs);
        if (isExpired) {
            hostRequestCard.classList.add("expired");
            hostRequestCard.title = 'This request is older than 3 months and is automatically rejected.';
        }
        ...
        if (isExpired) {
            ...
            badge.className = 'badge expired-badge';
            badge.textContent = 'Expired';
            badge.title = 'Submitted over 90 days ago';
...
function openModal(id) {
    ...
    const acceptButton = document.querySelector(".modal-actions .accept");
    const rejectButton = document.querySelector(".modal-actions .reject");

    // Show expired notice and disable accept if expired
    let expiredNotice = document.getElementById('expired-notice');
    if (!expiredNotice) {
        expiredNotice = document.createElement('div');
        expiredNotice.id = 'expired-notice';
        expiredNotice.className = 'expired-notice';
        const modalContent = modal.querySelector('.modal-content');
        if (modalContent) {
            modalContent.insertBefore(expiredNotice, modalContent.querySelector('.modal-actions'));
        }
    }
    if (isExpired) {
        expiredNotice.style.display = 'block';
        expiredNotice.textContent = 'This listing is older than 3 months and has been automatically rejected. Approval is disabled.';
        if (acceptButton) acceptButton.disabled = true;
        if (statusDropdown) statusDropdown.value = 'Rejected';
    } else {
        expiredNotice.style.display = 'none';
        if (acceptButton) acceptButton.disabled = false;
    }

    if (acceptButton) acceptButton.onclick = () => updateHostStatus(id, "Approved");
    if (rejectButton) rejectButton.onclick = () => updateHostStatus(id, "Rejected");
}
